...
const UpdateWebsiteForm = () => {
  ...
  const [existingThumbnailUrl, setExistingThumbnailUrl] = useState(""); // URL for existing thumbnail

  // State for form submission and data fetching status
  const [loading, setLoading] = useState(true);
  const [submitting, setSubmitting] = useState(false);
  const [error, setError] = useState(null);
  const [success, setSuccess] = useState(null);

  // Effect to fetch the existing website data
  useEffect(() => {
    const fetchWebsite = async () => {
      // Add a check to ensure websiteId is available before making the API call
      if (!websiteId) {
        setLoading(false);
        setError("Invalid website ID. Cannot fetch data.");
        return;
      }
      ...
      try {
        ...
        setFeatures(websiteData.features || []);
        setExistingThumbnailUrl(websiteData.thumbnail || "");
        
      } catch (err) {
        console.error("Error fetching website:", err);
        setError("Failed to load website data. Please check your network connection and backend endpoint.");
      } finally {
        setLoading(false);
      }
    };
    ...
    fetchWebsite();
  }, [websiteId]); // Re-run effect if websiteId changes
  ...
  const handleSubmit = async (e) => {
    ...
    if (thumbnail) {
      ...
      // If no new file, but an old one exists, keep the URL
      formData.append("existingThumbnailUrl", existingThumbnailUrl);
    }
    
    try {
      await api.put(`/websites/${websiteId}`, formData);
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-base-200 px-4">
      ...
      >
        <h2 className="text-2xl sm:text-3xl text-center font-bold text-white">Update Website</h2>
        ...
        <div>
          ...
          {existingThumbnailUrl && !thumbnail && (
            <div className="mb-2 text-sm text-gray-400">
              Current thumbnail: <span className="text-blue-400">{existingThumbnailUrl.split('/').pop()}</span>
            </div>
          )}
          <input
            type="file"
        ...
        <button
          ...
          className={`text-base sm:text-lg py-3 rounded-xl w-full transition ${
            submitting ? "bg-gray-500 cursor-not-allowed" : "bg-blue-600 text-slate-800 hover:bg-blue-900 hover:text-white"
          }`}
        >
          {submitting ? "Updating..." : "Update Website"}
        </button>
